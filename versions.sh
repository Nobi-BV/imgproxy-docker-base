export GOLANG_VERSION=1.27.1
export ZLIB_VERSION=2.3.3
export BROTLI_VERSION=1.2.0
export FFI_VERSION=3.8.0
export PCRE2_VERSION=10.48
export GLIB_VERSION=2.89.4
export HIGHWAY_VERSION=1.4.0
export QUANTIZR_VERSION=1.4.3
export LIBEXPAT_VERSION=2.8.4
export LIBXML2_VERSION=2.15.3
export LIBEXIF_VERSION=0.6.26
export LCMS2_VERSION=2.19.1
export LIBJPEGTURBO_VERSION=3.2.0
export LIBJXL_VERSION=0.12.0
export LIBPNG_VERSION=1.6.58
export LIBWEBP_VERSION=1.6.0
export LIBTIFF_VERSION=4.7.2
export CGIF_VERSION=0.5.4
export LIBDE265_VERSION=1.1.2
export KVAZAAR_VERSION=2.3.2
export DAV1D_VERSION=1.5.4
# export RAV1E_VERSION=0.5.1
export AOM_VERSION=3.15.0
export LIBHEIF_VERSION=1.23.3
export FREETYPE_VERSION=2.14.3
export FONTCONFIG_VERSION=2.18.3
export HARFBUZZ_VERSION=14.4.0
export PIXMAN_VERSION=0.46.4
export CAIRO_VERSION=1.18.4
export FRIBIDI_VERSION=1.0.16
export PANGO_VERSION=1.58.2
export LIBRSVG_VERSION=2.62.3
export VIPS_VERSION=8.18.6
